const CACHE='tomocron-v33-requisitos-renais-revisados-2026-08-20';
const CORE=['./','./index.html','./requirements-v2.html','./app.css?v=27','./app.js?v=27','./manifest.webmanifest','./icon-180.png','./icon-192.png','./icon-512.png','./logo-tomocron-final.jpg?v=24'];
...
async function fetchRequirementsV2(){
  try{return await fetch('./requirements-v2.html',{cache:'no-store'})}
  catch(_){return(await caches.match('./requirements-v2.html'))||new Response('Módulo de requisitos indisponível offline.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8'}})}
}
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/requirements.html')||url.pathname.endsWith('/requirements-v2.html')){
    event.respondWith(fetchRequirementsV2());
    return;
  }
